#!/usr/bin/env bash
# Delta virtual-prefix 8-GPU AWM OPSD run: 6 GPUs for the FSDP2 actor/rollout and two
# single-GPU teacher replicas for privileged-context top-k distillation.
...
ACTOR_MODEL=${ACTOR_MODEL:-/mnt/storage/disk1/verl_data/base_model/Qwen3.5-4B-DeltaVirtualPrefix-2048}
...
RUN_NAME=${RUN_NAME:-qwen3_5_4b_awm_opsd_delta_prefix_m2048}
...
prefix_config = config.get("delta_virtual_prefix", {})
if config.get("model_type") != "qwen3_5":
    raise SystemExit(f"Expected model_type=qwen3_5, got {config.get('model_type')!r}")
if config.get("architectures") != ["Qwen3_5DeltaVirtualPrefixForConditionalGeneration"]:
    raise SystemExit(f"Unexpected actor architecture: {config.get('architectures')!r}")
if prefix_config.get("num_virtual_tokens") != 2048:
    raise SystemExit(f"Expected 2048 virtual tokens, got {prefix_config.get('num_virtual_tokens')!r}")
weights_file = model / "delta_virtual_prefix.safetensors"
...
expected_shape = (2048, 2560)
if len(shapes) != 24 or any(shape != expected_shape for shape in shapes.values()):
    raise SystemExit(f"Expected 24 prefix tensors shaped {expected_shape}, got {shapes}")
if sum(shape[0] * shape[1] for shape in shapes.values()) != 125829120:
    raise SystemExit("Unexpected virtual-prefix parameter count")
...
print("Validated M=2048 actor model and qwen35_delta_virtual_prefix vLLM plugin")
PY
...
echo "Actor: $ACTOR_MODEL (24 x [2048, 2560] virtual-prefix tensors)"
...
python -m verl.trainer.main_ppo \
    ...
    'actor_rollout_ref.model.trainable_param_patterns=[".*[.]linear_attn[.]prefix_tokens$"]' \
    actor_rollout_ref.model.expected_trainable_param_count=24 \
    actor_rollout_ref.model.expected_trainable_numel=125829120 \
