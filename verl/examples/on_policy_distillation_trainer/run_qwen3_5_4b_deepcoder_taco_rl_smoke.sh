#!/usr/bin/env bash
# One real GRPO optimizer step through AWMAgentLoop and CodeJudge OpenEnv.
...
CODE_JUDGE_BASE_URL=${CODE_JUDGE_BASE_URL:-http://127.0.0.1:8901}
DATA_DIR=${DATA_DIR:-$REPO_ROOT/traj_data/deepcoder_taco_smoke}
TASK_IDX=${TASK_IDX:-2}
RUN_NAME=${RUN_NAME:-qwen3_5_4b_deepcoder_taco_rl_smoke}
...
if ! curl --fail --silent --show-error "$CODE_JUDGE_BASE_URL/health" >/dev/null; then
    echo "CodeJudge OpenEnv is not healthy at $CODE_JUDGE_BASE_URL" >&2
    exit 1
fi
...
python "$SCRIPT_DIR/prepare_deepcoder_taco_rl_data.py" \
    --data-dir "$REPO_ROOT/data/deepcoder_taco/177913a7bd43791646ef6a43645caa3c871ab3db" \
    --output-dir "$DATA_DIR" \
    --task-indices "$TASK_IDX" \
    --base-url "$CODE_JUDGE_BASE_URL" \
    --model "$MODEL" \
...
echo "DeepCoder TACO RL smoke: task_idx=$TASK_IDX, rollouts=$ROLLOUT_N, GPUs=$CUDA_VISIBLE_DEVICES"
echo "Log: $LOG_FILE"
...
python -m verl.trainer.main_ppo \
    ...
    trainer.project_name=self_evolver_deepcoder_taco \
    trainer.experiment_name="$RUN_NAME" \
    trainer.default_local_dir="$REPO_ROOT/checkpoints/self_evolver_deepcoder_taco/$RUN_NAME" \
