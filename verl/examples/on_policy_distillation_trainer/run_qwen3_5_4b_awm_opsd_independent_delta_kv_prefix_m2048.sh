#!/usr/bin/env bash
# Independent Delta K/V/beta/a Prefix 8-GPU AWM OPSD run: 6 GPUs for the
# FSDP2 actor/rollout and two teacher GPUs for privileged-context distillation.
...
ACTOR_MODEL=${ACTOR_MODEL:-/mnt/storage/disk1/verl_data/base_model/Qwen3.5-4B-IndependentDeltaKVPrefix-2048}
...
RUN_NAME=${RUN_NAME:-qwen3_5_4b_awm_opsd_independent_delta_kv_prefix_largelr}
...
prefix_config = config.get("independent_delta_kv_prefix", {})
if config.get("model_type") != "qwen3_5":
    raise SystemExit(f"Expected model_type=qwen3_5, got {config.get('model_type')!r}")
if config.get("architectures") != ["Qwen3_5IndependentDeltaKVPrefixForConditionalGeneration"]:
...
print("Validated M=2048 Independent Delta actor and native vLLM plugin")
PY
...
echo "Actor: $ACTOR_MODEL (96 Independent Delta K/V/beta/a prefix tensors; 305,135,616 parameters)"
...
python -m verl.trainer.main_ppo \
    ...
    'actor_rollout_ref.model.trainable_param_patterns=[".*[.]linear_attn[.](prefix_k|prefix_v|prefix_beta_logits|prefix_a)$"]' \
    actor_rollout_ref.model.expected_trainable_param_count=96 \
    actor_rollout_ref.model.expected_trainable_numel=305135616 \
