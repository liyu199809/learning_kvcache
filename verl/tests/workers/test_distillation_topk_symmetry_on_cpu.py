...
def test_topk_loss_routes_fsdp_family_to_fsdp_implementation(strategy):
    expected = {"distillation_losses": torch.ones(1, 2)}
    with patch(
        ...
        output = compute_topk_loss(
            ...
            data=TensorDict(
                {
                    "teacher_logprobs": torch.zeros(1, 2, 1),
                    "teacher_ids": torch.zeros(1, 2, 1, dtype=torch.long),
                },
                batch_size=[1, 2],
            ),
            student_logits=torch.zeros(1, 2, _VOCAB_SIZE),
            data_format="bshd",
        )
