...
def test_slice_privileged_teacher_keeps_causal_response_rows_and_pads_boundaries():
    priv_len, k = 10, 2
    ...
    # The final teacher row is a server-added dummy. The two rows before it
    # predict the two response tokens and belong at student logits positions
    # prompt_len - 1 and prompt_len.
    assert torch.equal(ids[1:3], teacher_ids[-3:-1])
    assert torch.equal(logprobs[1:3], teacher_logprobs[-3:-1])
    assert torch.all(ids[[0, 3]] == 0)
    assert torch.all(logprobs[[0, 3]] == 0.0)
...
def test_slice_response_length_zero_is_empty_not_whole_tensor():
    ...
    # response must yield only padded rows (including the trailing dummy).
...
def test_privileged_slice_aligns_response_rows_after_padding():
    """End-to-end alignment with the causal left shift used by the loss path."""
    ...
    for j in range(resp_len):
        # Logits row prompt_width+j-1 predicts response token j. The teacher
        # extractor uses the same layout, at privileged row Q+j-1.
        assert torch.equal(padded[prompt_width + j - 1], teacher_ids[prompt_len + block_len + j - 1])
...
def test_build_privileged_chat_turn_places_solution_in_user_turn():
    ...
    assert tok.last_kwargs == {"enable_thinking": True, "return_dict": False}
