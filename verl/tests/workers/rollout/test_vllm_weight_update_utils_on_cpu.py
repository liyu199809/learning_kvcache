...
def test_vllm_update_weights_validates_virtual_prefix_partial_load():
    model = _ToyModel()
    ...
    class _Mapper:
        ...
        def apply_list(names):
            return [name.replace("model.language_model.", "language_model.model.") for name in names]

    model.hf_to_vllm_mapper = _Mapper()
    model.load_weights = lambda weights: {"language_model.model.layers.0.linear_attn.prefix_tokens"}
    worker = object.__new__(vLLMColocateWorkerExtension)
    worker.model_runner = _FakeModelRunner(model)

    worker._update_weights(
        [(prefix_name, torch.zeros(2, 4))],
        peft_config=None,
        base_sync_done=True,
    )


def test_vllm_update_weights_rejects_missing_virtual_prefix_partial_load():
    import pytest

    model = _ToyModel()
    prefix_name = "model.language_model.layers.0.linear_attn.prefix_tokens"
    model.load_weights = lambda weights: set()
    worker = object.__new__(vLLMColocateWorkerExtension)
    worker.model_runner = _FakeModelRunner(model)

    with pytest.raises(ValueError, match="failed to load virtual-prefix"):
        worker._update_weights(
            [(prefix_name, torch.zeros(2, 4))],
            peft_config=None,
            base_sync_done=True,
