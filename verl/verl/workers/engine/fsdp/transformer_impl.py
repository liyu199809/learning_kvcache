...
class FSDPEngine(BaseEngine):
    ...
    def __init__(
        ...
        self._trainable_param_order: tuple[str, ...] = ()
    ...
    def initialize(self):
        ...
        self.checkpoint_manager = FSDPCheckpointManager(
            ...
            trainable_param_names=self._trainable_param_order,
            base_model_path=self.model_config.local_path,
        )
    ...
    def _build_optimizer(self, module):
        ...
        if not trainable_parameters:
            raise ValueError("No trainable parameters remain after model parameter selection")
        optimizer = build_optimizer(trainable_parameters, self.optimizer_config)

        return optimizer

    def _configure_trainable_parameters(self, module: torch.nn.Module) -> None:
        patterns = list(self.model_config.trainable_param_patterns or [])
        if not patterns:
            if self.model_config.rollout_sync_trainable_only:
                raise ValueError(
                    "rollout_sync_trainable_only=True requires non-empty trainable_param_patterns"
                )
            self._trainable_param_order = tuple(
                name for name, param in module.named_parameters() if param.requires_grad
            )
            self._trainable_param_names = set(self._trainable_param_order)
            return
        if self._is_lora:
            raise ValueError("trainable_param_patterns cannot be combined with PEFT LoRA")
        ...
        for pattern in patterns:
            try:
                ...
                raise ValueError(f"Invalid trainable parameter pattern {pattern!r}: {error}") from error

        selected: dict[str, torch.nn.Parameter] = {}
        for name, param in module.named_parameters():
            is_selected = any(pattern.fullmatch(name) for pattern in compiled)
            param.requires_grad_(is_selected)
            if is_selected:
                selected[name] = param

        expected_count = self.model_config.expected_trainable_param_count
        expected_numel = self.model_config.expected_trainable_numel
        selected_numel = sum(param.numel() for param in selected.values())
        if expected_count is not None and len(selected) != expected_count:
            raise ValueError(
                f"Selected {len(selected)} trainable tensors, expected {expected_count}; "
                f"patterns={patterns!r}, names={sorted(selected)}"
            )
        if expected_numel is not None and selected_numel != expected_numel:
            raise ValueError(
                f"Selected {selected_numel} trainable elements, expected {expected_numel}; "
                f"patterns={patterns!r}"
            )
        if not selected:
            raise ValueError(f"trainable_param_patterns matched no parameters: {patterns!r}")

        # FSDP's sharded backward hooks need a gradient-carrying activation at
        # the model boundary when the embedding and almost all base parameters
        # are frozen. PEFT LoRA enables the same hook for this reason.
        if hasattr(module, "enable_input_require_grads"):
            module.enable_input_require_grads()

        self._trainable_param_names = set(selected)
        self._trainable_param_order = tuple(selected)
        if self.rank == 0:
            logger.info(
                "Selected %d trainable tensors (%d elements): %s",
                len(selected),
                selected_numel,
                {name: tuple(param.shape) for name, param in selected.items()},
            )

    def _filter_trainable_state_dict(self, params):
        if not self._trainable_param_names:
            raise ValueError("Trainable-only state requested without selected trainable parameter names")
        filtered = type(params)(
            (name, param) for name, param in params.items() if name in self._trainable_param_names
        )
        missing = self._trainable_param_names.difference(filtered)
        if missing:
            raise ValueError(
                "FSDP state dict is missing selected trainable parameters: " + ", ".join(sorted(missing))
            )
        return filtered
    ...
    def _build_model_optimizer(self):
        ...
        self._configure_trainable_parameters(module)
    ...
    def get_per_tensor_param_shard(self, trainable_only=False, **kwargs):
        ...
        if trainable_only:
            params = self._filter_trainable_state_dict(params)
    ...
    def get_per_tensor_param(self, layered_summon=False, base_sync_done=False, trainable_only=False, **kwargs):
        ...
        if trainable_only:
            if peft_config is not None:
                raise ValueError("Generic trainable-only sync cannot be combined with PEFT adapter sync")
            params = self._filter_trainable_state_dict(params)
...
class FSDPEngineWithLMHead(FSDPEngine):
    def prepare_model_inputs(self, micro_batch: TensorDict):
        ...
        if use_remove_padding:
            ...
            packed_sequence_kwargs = {}
            if not self.use_ulysses_sp:
                # Keep explicit segment metadata alongside the flattened token
                # stream. FlashAttention can infer boundaries from reset
                # position ids, but recurrent/causal-convolution mixers such as
                # Qwen3.5 GatedDeltaNet require cu_seqlens/seq_idx directly.
                cu_seq_lens = micro_batch["input_ids"].offsets().to(
                    device=input_ids_rmpad.device,
                    dtype=torch.int32,
                )
                sequence_lengths = cu_seq_lens[1:] - cu_seq_lens[:-1]
                packed_sequence_kwargs = {
                    "cu_seq_lens_q": cu_seq_lens,
                    "cu_seq_lens_k": cu_seq_lens,
                    "max_length_q": int(sequence_lengths.max().item()),
                    "max_length_k": int(sequence_lengths.max().item()),
    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict, logits_processor_func):
        ...
        if use_remove_padding:
            ...
            if use_fused_kernels:
                ...
                if calculate_entropy:
                    if self.engine_config.entropy_from_logits_with_chunking:
                        entropy_input = logits.reshape(-1, logits.shape[-1])

                        def _chunked_entropy(tensor):
                            return self.compute_entropy_from_logits(
                                tensor,
                                chunk_size=self.engine_config.entropy_from_logits_chunk_size,
                            )

                        if not self.engine_config.entropy_checkpointing:
                            entropy = _chunked_entropy(entropy_input)
                        else:
                            entropy = torch.utils.checkpoint.checkpoint(_chunked_entropy, entropy_input)
