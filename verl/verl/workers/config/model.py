...
class HFModelConfig(BaseConfig):
    ...
    # Optional regular expressions selecting the only parameters that remain
    # trainable. Patterns are matched with re.fullmatch against named_parameters().
    trainable_param_patterns: list[str] = field(default_factory=list)
    expected_trainable_param_count: Optional[int] = None
    expected_trainable_numel: Optional[int] = None
    rollout_sync_trainable_only: bool = False
