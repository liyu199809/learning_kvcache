    ...
    def _update_weights(
        ...
        if peft_config and base_sync_done:
            ...
            if is_fp8_model(self.model_runner.vllm_config):
                ...
                if param_updates:
                    for model in self._iter_all_models():
                        loaded = model.load_weights(param_updates)
                        loaded_params = set(loaded or ())
                        expected_prefix_params = {
                            name for name, _ in param_updates if name.endswith(".linear_attn.prefix_tokens")
                        }
                        weights_mapper = getattr(model, "hf_to_vllm_mapper", None)
                        if weights_mapper is not None and hasattr(weights_mapper, "apply_list"):
                            expected_loaded_prefix_params = set(
                                weights_mapper.apply_list(sorted(expected_prefix_params))
                            )
                        else:
                            expected_loaded_prefix_params = expected_prefix_params
                        missing_prefix_params = expected_loaded_prefix_params.difference(loaded_params)
                        if missing_prefix_params:
                            raise ValueError(
                                "vLLM failed to load virtual-prefix parameters: "
                                + ", ".join(sorted(missing_prefix_params))
                            )
                loaded_buffers = self._apply_buffer_updates_all_models(buffer_updates, named_buffers)
                logger.info(
