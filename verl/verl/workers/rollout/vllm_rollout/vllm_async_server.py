...
class vLLMHttpServer:
    ...
    async def _sleep_hybrid(self):
        """HYBRID sleep: partial updates need level=1; full weights may use level=2.

        Adapter and trainable-only updates cannot reconstruct frozen base weights
        after level=2 sleep discards them, so they retain a restorable weight copy.
        ...
        # Trainable-only sync likewise updates only a parameter subset and cannot
        # reconstruct frozen base weights after level 2 sleep.
        # vllm_ascend not support sleep_level now. Enabling EP during training may lead to accuracy issues.
        trainable_only_update = bool(getattr(self.model_config, "rollout_sync_trainable_only", False))
        if (
            mtp_rollout_enabled
            or self.lora_as_adapter
            or trainable_only_update
