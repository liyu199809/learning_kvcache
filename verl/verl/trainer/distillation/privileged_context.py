...
def build_privileged_chat_turn(
    ...
    # Some tokenizers (notably the current Qwen3.5 tokenizer) default to
    # returning a BatchEncoding here. ``list(BatchEncoding)`` yields field
    # names instead of token ids, so require the plain token-id list.
...
def slice_privileged_teacher_to_student(
    ...
    The teacher server already stores each next-token distribution at the causal
    logit position that predicts it: row ``i`` predicts token ``i + 1``. Its final
    row is therefore a dummy. For a response of length ``R``, the useful teacher
    rows are the ``R`` rows immediately before that dummy row.

    The student loss uses the same causal layout and later selects model-output
    rows ``student_prompt_length - 1`` through ``... + R - 1``. Put the useful
    teacher rows at exactly those positions, pad the earlier prompt rows, and
    retain one trailing dummy row so the result still has the same length as the
    student's ``prompt + response`` sequence.
    ...
    if student_prompt_length <= 0:
        raise ValueError(f"student_prompt_length must be positive, got {student_prompt_length}")
    if pad_token_id is None:
        pad_token_id = 0
    k = teacher_ids.shape[-1]
    # The server output has a trailing dummy row. Take the R causal-logit rows
    # immediately before it; spelling out both bounds also handles R == 0.
    response_start = teacher_ids.shape[0] - response_length - 1
    response_end = teacher_ids.shape[0] - 1
    if response_start < 0:
        raise ValueError(
            f"Teacher output length {teacher_ids.shape[0]} is too short for "
            f"response_length={response_length} plus its trailing dummy row."
        )
    response_ids = teacher_ids[response_start:response_end]
    response_logprobs = teacher_logprobs[response_start:response_end]
    prompt_ids = torch.full(
        (student_prompt_length - 1, k), int(pad_token_id), dtype=teacher_ids.dtype, device=teacher_ids.device
    )
    prompt_logprobs = torch.zeros(
        (student_prompt_length - 1, k), dtype=teacher_logprobs.dtype, device=teacher_logprobs.device
    )
    trailing_ids = torch.full(
        (1, k), int(pad_token_id), dtype=teacher_ids.dtype, device=teacher_ids.device
    )
    trailing_logprobs = torch.zeros(
        (1, k), dtype=teacher_logprobs.dtype, device=teacher_logprobs.device
    )
    ids = torch.cat([prompt_ids, response_ids, trailing_ids], dim=0)
    logprobs = torch.cat([prompt_logprobs, response_logprobs, trailing_logprobs], dim=0)
    return ids, logprobs
