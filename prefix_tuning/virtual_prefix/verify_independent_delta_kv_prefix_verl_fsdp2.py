#!/usr/bin/env python3
"""Two-rank FSDP2 smoke for Independent Delta prefix training."""
...
MODEL = os.getenv(
    "MODEL",
    "/mnt/storage/disk1/verl_data/base_model/Qwen3.5-4B-IndependentDeltaKVPrefix-2048",
)
PREFIX_PATTERN = r".*\.linear_attn\.(prefix_k|prefix_v|prefix_beta_logits|prefix_a)$"
EXPECTED_COUNT = 96
EXPECTED_NUMEL = 305135616
...
def main() -> None:
    ...
    for name, parameter in trainable.items():
        ...
        if finite.item() and nonzero.item():
            step1_k_tensors += 1
    if step1_k_tensors != 24:
        raise AssertionError(f"Only {step1_k_tensors}/24 prefix_k tensors had finite nonzero step-1 gradients")
    ...
    if peft_config is not None or len(synced) != EXPECTED_COUNT:
        raise AssertionError("Trainable-only rollout sync did not return exactly 96 standard tensors")
    ...
    checkpoint_dir = os.getenv(
        "CHECKPOINT",
        os.path.join(tempfile.gettempdir(), "verl_independent_delta_kv_prefix_fsdp2_smoke"),
    )
    ...
    if rank == 0:
        ...
        if "modeling_qwen3_5_independent_delta_kv_prefix.py" not in hf_files:
        ...
        print("verl_independent_delta_kv_prefix_fsdp2_ok", True)
