#!/usr/bin/env python3
"""Two-rank FSDP2 smoke for hybrid Delta + attention prefix training."""
...
MODEL = os.getenv(
    "MODEL",
    "/mnt/storage/disk1/verl_data/base_model/Qwen3.5-4B-HybridDeltaResidualAttentionPrefix-G2048-A256",
)
PREFIX_PATTERN = (
    r".*\.(linear_attn\.(prefix_k|prefix_v|prefix_beta_logits|prefix_a)"
    r"|self_attn\.(prefix_key_tokens|prefix_value_tokens))$"
)
EXPECTED_COUNT = 112
EXPECTED_NUMEL = 315621376
...
def main() -> None:
    ...
    prefix_name = next(name for name in sorted(trainable) if name.endswith(".prefix_k"))
    ...
    def forward_loss():
        ...
        return output.logits[:, -1, 123].float().mean()

    loss = forward_loss()
    loss.backward()
    step1_delta_k_tensors = 0
    step1_attention_value_tensors = 0
    step1_attention_key_tensors = 0
    nonfinite_step1_tensors = []
    for name, parameter in trainable.items():
        ...
        if grad is not None and not finite.item():
            nonfinite_step1_tensors.append(name)
        is_nonzero = bool(finite.item() and nonzero.item())
        if name.endswith(".linear_attn.prefix_k") and is_nonzero:
            step1_delta_k_tensors += 1
        elif name.endswith(".self_attn.prefix_value_tokens") and is_nonzero:
            step1_attention_value_tensors += 1
        elif name.endswith(".self_attn.prefix_key_tokens") and is_nonzero:
            step1_attention_key_tensors += 1
    if step1_delta_k_tensors != 24:
        raise AssertionError(
            f"Only {step1_delta_k_tensors}/24 Delta prefix_k tensors had nonzero step-1 gradients"
        )
    if step1_attention_value_tensors != 8:
        raise AssertionError(
            f"Only {step1_attention_value_tensors}/8 attention prefix_value tensors had nonzero step-1 gradients"
        )
    if step1_attention_key_tensors:
        raise AssertionError(
            f"{step1_attention_key_tensors}/8 attention key tables moved while V=0"
        )
    if nonfinite_step1_tensors:
        raise AssertionError("Non-finite step-1 gradients: " + ", ".join(nonfinite_step1_tensors))
    engine.optimizer.step()
    engine.optimizer.zero_grad(set_to_none=True)

    second_loss = forward_loss()
    second_loss.backward()
    step2_nonzero_tensors = 0
    for parameter in trainable.values():
        ...
        if finite.item() and nonzero.item():
            step2_nonzero_tensors += 1
    if step2_nonzero_tensors != EXPECTED_COUNT:
        raise AssertionError(
            f"Only {step2_nonzero_tensors}/{EXPECTED_COUNT} prefix tensors had finite nonzero step-2 gradients"
        )
    engine.optimizer.step()
    engine.optimizer.zero_grad(set_to_none=True)
    ...
    if peft_config is not None or len(synced) != EXPECTED_COUNT:
        raise AssertionError("Trainable-only rollout sync did not return exactly 112 standard tensors")
    ...
    checkpoint_dir = os.getenv(
        "CHECKPOINT",
        os.path.join(tempfile.gettempdir(), "verl_hybrid_delta_residual_attention_prefix_fsdp2_smoke"),
    )
    ...
    if rank == 0:
        ...
        if metadata.get("trainable_type") != "hybrid_delta_residual_attention_prefix":
            raise AssertionError(f"Unexpected checkpoint trainable_type: {metadata.get('trainable_type')}")
        if metadata.get("num_virtual_tokens") != 2048:
            raise AssertionError("Checkpoint is missing the Delta prefix length")
        if metadata.get("attention_num_virtual_tokens") != 256:
            raise AssertionError("Checkpoint is missing the attention prefix length")
        ...
        if "modeling_qwen3_5_hybrid_delta_residual_attention_prefix.py" not in hf_files:
        ...
        print(
            {
                ...
                "step2_loss": float(second_loss.detach()),
                "checkpoint": checkpoint_dir,
            }
        )
        print("verl_hybrid_delta_residual_attention_prefix_fsdp2_ok", True)
