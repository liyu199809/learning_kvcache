...
export OPD_EVOLVER_EMBEDDING_DEVICE="${OPD_EVOLVER_EMBEDDING_DEVICE:-cuda}"
export OPD_EVOLVER_EMBEDDING_MAX_LENGTH="${OPD_EVOLVER_EMBEDDING_MAX_LENGTH:-2048}"
export OPD_EVOLVER_EMBEDDING_DTYPE="${OPD_EVOLVER_EMBEDDING_DTYPE:-float16}"
...
TEACHER_MODE="${TEACHER_MODE:-gold_sql}"

if [ "${GOLD_ONLY:-0}" = "1" ]; then
  TEACHER_MODE="gold_sql"
  MEMORY_DIR=""
  MEMORY_TOP_K="0"
  MEMORY_RETRIEVE_K="0"
  MEMORY_SELECT_K="0"
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/opsd_sql}"
DATASET="${DATASET:-data/sql/spider/ic_spider_dev.json}"

MAX_TRAIN_SAMPLES="${MAX_TRAIN_SAMPLES:-}"

SUCCESS_FILTER_CSV="${SUCCESS_FILTER_CSV:-}"
...
CMD=(
  ...
  scripts/train/opsd_mem/train_opsd_sql.py
...
[ -n "${SUCCESS_FILTER_CSV}" ] && CMD+=(--success_filter_csv "${SUCCESS_FILTER_CSV}")

echo "================================================================"
echo "  SQL OPD Training"
echo "  Model:     ${MODEL_DIR##*/}"
echo "  Dataset:   ${DATASET}"
echo "  GPUs:      ${CUDA_VISIBLE_DEVICES}"
echo "  vLLM:      ${USE_VLLM} (colocate, TP=2, util=${VLLM_GPU_UTIL})"
echo "  LoRA:      r=${LORA_R} α=${LORA_ALPHA}"
echo "  Batch:     ${BS}×${GA}×2 = $((BS * GA * 2)) effective"
...
if [ -n "${SUCCESS_FILTER_CSV}" ]; then
  echo "  Success filter: ${SUCCESS_FILTER_CSV}  (CAUTION: see comments in script)"
fi
if [ -n "${MEMORY_DIR}" ]; then
  echo "  Memory:    retrieve_k=${MEMORY_RETRIEVE_K} → select_k=${MEMORY_SELECT_K}"
  echo "  Min score: ${MEMORY_MIN_SCORE}  Teacher mode: ${TEACHER_MODE}"
else
  echo "  Teacher:   ${TEACHER_MODE} (gold-only)"
fi
echo "================================================================"
