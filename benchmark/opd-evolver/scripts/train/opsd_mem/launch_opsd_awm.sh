...
DATASET_PATH="${DATASET_PATH:-workspace/awm_runs_scale_3000_memory/executor_opd_tasks.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/opsd_awm/executor}"
...
export WANDB_PROJECT="${WANDB_PROJECT:-awm-executor-opsd}"
...
RUN_NAME="${RUN_NAME:-awm_executor_opsd_e${EPOCHS}_lr${LR}}"
MAX_TRAIN_SAMPLES="${MAX_TRAIN_SAMPLES:-}"

MEMORY_DIR="${MEMORY_DIR:-workspace/memory/awm_scale_3000}"
...
CMD=(
  ...
  scripts/train/opsd_mem/train_opsd_awm.py
...
[ -n "${RUN_NAME}" ] && CMD+=(--run_config "${RUN_NAME}" --run_name "${RUN_NAME}")
