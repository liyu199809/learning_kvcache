...
from trl.experimental.utils import DataCollatorForChatML, empty_cache, truncate_dataset
...
class OPSDTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        self.use_transformers_paged = bool(getattr(args, "use_transformers_paged", False))
        ...
        if self.use_vllm:
            ...
            if self.vllm_mode == "server":
                ...
                ensure_master_addr_port()
                _disable_car = os.environ.get("VLLM_DISABLE_CUSTOM_ALL_REDUCE", "").strip().lower() in (
                    "1",
                    "true",
                    "yes",
                )
            ...
            self.vllm_sync_frequency = getattr(args, "vllm_sync_frequency", 1)
    ...
    def _generate_on_policy_outputs_vllm(self, inputs, generation_config, pad_token_id=None):
        ...
        prompt_attention_mask = inputs.get("student_prompt_attention_mask")
        if prompt_attention_mask is None:
            prompt_attention_mask = torch.ones_like(prompt_ids, device=device)
        else:
            prompt_attention_mask = prompt_attention_mask.to(device)
        completion_pad_id = pad_token_id
        if completion_pad_id is None:
            completion_pad_id = getattr(self.processing_class, "eos_token_id", 0)
        ...
        for completion_tensor in completion_ids_tensors:
            if len(completion_tensor) > max_completion_length:
                ...
                completion_attention_mask_list.append(
                    torch.ones((max_completion_length,), device=device, dtype=prompt_attention_mask.dtype)
                )
                ...
                padded_tensor = torch.cat(
                    [
                        completion_tensor,
                        torch.full(
                            (padding_needed,), completion_pad_id, device=device, dtype=completion_tensor.dtype
                        ),
                    ]
                )
                ...
                completion_attention_mask_list.append(
                    torch.cat(
                        [
                            torch.ones((len(completion_tensor),), device=device, dtype=prompt_attention_mask.dtype),
                            torch.zeros((padding_needed,), device=device, dtype=prompt_attention_mask.dtype),
                        ]
                    )
                )
            else:
        ...
        completion_attention_mask = torch.stack(completion_attention_mask_list)
