...
# LLM judge budget / timeout. The default max_completion_tokens is sized for
# reasoning models (e.g. ARK doubao-thinking endpoints), whose thinking phase
# can consume several thousand tokens before emitting the JSON verdict -- the
# previous 4096 ceiling was eaten entirely by reasoning and yielded empty output.
# Non-reasoning models simply use less; raising the ceiling is safe for both.
LLM_JUDGE_MAX_COMPLETION_TOKENS = int(
    os.environ.get("OPENENV_AWM_LLM_JUDGE_MAX_TOKENS", "16384")
)
LLM_JUDGE_TIMEOUT_S = float(os.environ.get("OPENENV_AWM_LLM_JUDGE_TIMEOUT", "300.0"))
...
async def run_llm_judge(
    ...
    try:
        ...
        # Stream the response. Reasoning models (e.g. ARK doubao-thinking) spend a
        # long time in the thinking phase before emitting any output; a non-streamed
        # request leaves the connection idle for minutes and ARK's server-side
        # gateway resets it (~180s). Streaming keeps the connection alive via
        # incremental chunks and is the only robust path for such models.
        async def _stream_and_collect() -> str:
            stream = await client.chat.completions.create(
                ...
                temperature=1.0,
                max_completion_tokens=LLM_JUDGE_MAX_COMPLETION_TOKENS,
        ...
        try:
            content = await asyncio.wait_for(
                _stream_and_collect(),
                timeout=LLM_JUDGE_TIMEOUT_S + 30.0,
            )
        except asyncio.TimeoutError:
            return "judge_error", {
                "error": f"LLM judge timed out after {LLM_JUDGE_TIMEOUT_S}s"
            }
